...
public class SVGPathFigure extends AbstractAttributedCompositeFigure implements SVGFigure {
    ...
    private void drawCaps(Graphics2D g) {
    	if (children.size()>0) {
    		SVGBezierFigure f = (SVGBezierFigure) children.get(children.size() - 1);
    		BezierPath p = f.getBezierPath();
    		if (get(END_DECORATION) != null && p.size() > 1) {
        		Point2D.Double p1 = p.get(p.size() - 1, 0);
                Point2D.Double p2 = p.get(p.size() - 2, 0);
    ...
    public Collection<Action> getActions(Point2D.Double p) {
        final ResourceBundleUtil labels = ResourceBundleUtil.getBundle("de.rwth.oosc.figures.svg.Labels");
