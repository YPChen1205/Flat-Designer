...
public class IOUtil {
	...
	public static FurnitureModel loadDefaultModel() {
		...
		try {
			model = load(new File(IOUtil.class.getResource(CUSTOM_FURNITURE_PATH).toURI()).getAbsolutePath());
	...
	public static FurnitureModel load(String path) throws URISyntaxException {
		...
		for (File catDir : dirStructure.listFiles()) {
			if (catDir.isDirectory()) {
				...
				try {
					model.addFurnitures(category, loadFurnituresByCatalogPath(dirStructure.getAbsolutePath() + File.separator + category));;
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
...
//	}
	...
	public static List<CustomFurniture> loadFurnituresByCatalogPath(String catalogPath) throws URISyntaxException, IOException {
	...
	public static CustomFurniture loadFurniture(File furnitureFile, File iconFile) throws IOException, URISyntaxException {
	...
	private static void saveAsIcon(String categoryPath, CustomFurniture furniture, double dimension) throws FileNotFoundException, IOException, URISyntaxException {
	...
	public static void saveFurniture(String categoryPath, CustomFurniture furniture) throws FileNotFoundException, IOException, URISyntaxException {
		Drawing drawing = new DefaultDrawing();
		...
		drawing.getOutputFormats().get(0).write(new BufferedOutputStream(
				new FileOutputStream(
						new File(getXMLPath(categoryPath, furniture.getName())))), drawing);
